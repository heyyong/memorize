import { get } from '@/underlie/oxford/req';
...
export async function getWord(context: IContext, lang: string, word: string): Promise<RetrieveEntry> {
    try {
        context.log.debug(`start to request getWord(${lang}, ${word})`)
        const { data } = await get(`/api/v2/words/${lang}?q=${word}`);
        context.log.debug(`finish to request getWord(${lang}, ${word})`)
